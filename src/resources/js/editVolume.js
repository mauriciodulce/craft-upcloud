$(document).ready(function () {
  const $upcloudEndpoint = $('.upcloud-endpoint');
  ...
  $s3RefreshBucketsBtn.click(function () {
    ...
    const data = {
      endpoint: $upcloudEndpoint.val(),
      keyId: $s3AccessKeyIdInput.val(),
      secret: $s3SecretAccessKeyInput.val(),
    };

    Craft.sendActionRequest('POST', 'upcloud/buckets/load-bucket-data', {data})
      .then(({data}) => {
  ...
  const maybeUpdateUrl = function () {
    const endpoint = $upcloudEndpoint.val();
    const bucket = $manualBucket.val();
    
    if ($hasUrls.val() && endpoint.length && bucket.length) {
      ...
      try {
        ...
        // If not a valid URL, try to extract hostname
        endpointHost = endpoint.replace(/^https?:\/\//, '').replace(/\/$/, '');
      }
      
      // UpCloud uses virtual-hosted-style URLs: https://bucket.endpoint.com/
      $fsUrl.val('https://' + bucket + '.' + endpointHost + '/');
    }
  };

  $upcloudEndpoint.keyup(maybeUpdateUrl);
